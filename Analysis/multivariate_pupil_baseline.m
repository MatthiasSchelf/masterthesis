% Function to get the baseline values using the multivariate probability
% distribution across trials for the pupillometry.
function time_resolved_distributions_pupil_baseline = multivariate_pupil_baseline(baseline_blocks_of_five, baseline_blocks_of_nine, baseline_blocks_of_thirteen)
    ...
    function interpolated_segments = filter_and_interpolate_segments(baseline_blocks)
        % Filter out blocks shorter than 15 (since there was one trial in
        % which there were a very limited amount of timepoints (= outlier).
        ...
        % Interpolate segments to a fixed length of 16 (no equal length =
        % error)
        fixed_length = 16;
    ...
    % Initialize a cell array to store results later
    ...
    for t = 1:num_time_points
        ...
        % Store in the previously made cell array
