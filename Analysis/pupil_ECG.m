...
% Initialize structures to store restults for every single participant
participant_results = struct();

% Loop for each participant
for i = 1:length(participants)
    participant = participants{i};
    participant_field_name = strrep(participant, '-', '_'); % '_' is needed for storing results. 
    ...
    II_average = calculateII_different(difference_time_series_ecg, difference_time_series_pupil);
    ...
    II_multivariate = calculateII_different(multivariate_difference_pupil, multivariate_difference_ecg);

    % Store results in the structure
    participant_results.(participant_field_name).II_pupil_ecg_average = II_average;
    participant_results.(participant_field_name).II_pupil_ecg_multivariate = II_multivariate;
